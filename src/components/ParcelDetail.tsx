...
const StatusPill: React.FC<{ status: string }> = ({ status }) => {
  ...
  const map: Record<string, { cls: string; label: string }> = {
    ...
    running: { cls: 'bg-nkz-info-soft text-nkz-info', label: t('parcelDetail.statusRunning', 'Running') },
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-xs font-semibold border transition-colors ${
                  on
                    ...
                    : 'bg-white text-slate-600 border-slate-300 hover:border-nkz-accent-base'
                }`}
              >
                {isSar ? '🛰️ SAR' : idx}
              </button>
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {overview.active_jobs_count > 0 && (
            <span className="inline-flex items-center gap-1 text-[11px] text-nkz-info bg-nkz-info-soft px-2 py-0.5 rounded-full">
        ...
        <div className="sticky bottom-4 z-10 flex items-center justify-between gap-3 bg-white border border-slate-300 rounded-xl px-4 py-3 shadow-lg max-w-4xl mx-auto">
          <div className="flex items-center gap-3 text-sm">
            <button
              ...
              className="text-[11px] text-nkz-info hover:underline"
            >
